(function () {
  ...
  class QuizHelper {
    ...
    async simulateHumanClick(element) {
      if (!element || !this.autoMode) return false;
      ...
      try {
        ...
        // Tạo quỹ đạo di chuyển chuột mô phỏng (từ 1 vị trí nào đó trên màn hình)
        const startX = targetX - (Math.random() * 200 + 100) * (Math.random() > 0.5 ? 1 : -1);
        const startY = targetY - (Math.random() * 200 + 100) * (Math.random() > 0.5 ? 1 : -1);
        const steps = 15;

        const dispatchPointerAndMouse = (type, x, y, buttons = 0) => {
          const pointerType = type.replace('mouse', 'pointer');
          // Pointer Event dành cho các web đời mới / React / ReCaptcha
          if (window.PointerEvent) {
            const pev = new PointerEvent(pointerType, {
              bubbles: true, cancelable: true, view: window,
              clientX: x, clientY: y, screenX: x, screenY: y,
              buttons: buttons, pointerId: 1, pointerType: 'mouse', isPrimary: true
            });
            element.dispatchEvent(pev);
          }
          // Mouse Event chuẩn
          const mev = new MouseEvent(type, {
            bubbles: true, cancelable: true, view: window,
            clientX: x, clientY: y, screenX: x, screenY: y,
            buttons: buttons
          });
          element.dispatchEvent(mev);
        };

        // Bắn sự kiện di chuyển chuột (Mô phỏng đường cong chuột của người thật)
        for (let i = 1; i <= steps; i++) {
          const progress = i / steps;
          // Easing function outQuad để nó lướt mượt hơn
          const easeOut = progress * (2 - progress);
          const currX = startX + (targetX - startX) * easeOut;
          const currY = startY + (targetY - startY) * easeOut + (Math.sin(progress * Math.PI) * 15); // Đường hơi cong

          dispatchPointerAndMouse('mousemove', currX, currY, 0);
          await delay(10 + Math.random() * 15);
        }

        dispatchPointerAndMouse('mouseover', targetX, targetY, 0);
        dispatchPointerAndMouse('mouseenter', targetX, targetY, 0);

        await delay(50 + Math.random() * 100);
        dispatchPointerAndMouse('mousedown', targetX, targetY, 1);

        await delay(30 + Math.random() * 80);
        dispatchPointerAndMouse('mouseup', targetX, targetY, 0);

        // Không gọi element.click() trực tiếp vì dễ bị hook, mà bắn Event click
        const clickEv = new MouseEvent('click', {
          bubbles: true, cancelable: true, view: window,
          clientX: targetX, clientY: targetX, screenX: targetX, screenY: targetY, buttons: 0
        });
        element.dispatchEvent(clickEv);

        return true;
      } catch (err) {
        console.error("simulateHumanClick error", err);
        return false;
      }
    }
        ...
        if (response?.found && response.data?.correctAnswer) {
          ...
          if (shouldSelectWrong) {
            ...
            if (questionData.type === "simulation") {
              ...
              return await this.autoSelectWrongMultipleChoiceAnswer(correctAnswer);
            }
          } else {
            if (questionData.type === "simulation") {
              return this.autoSelectSimulationAnswer(correctAnswer);
            } else {
              return await this.autoSelectMultipleChoiceAnswer(correctAnswer);
            }
          }
        } else {
          ...
          if (questionData.type === "simulation") {
            ...
            return await this.autoSelectRandomMultipleChoiceAnswer();
          }
        }
      } catch (error) {
    ...
    async autoSelectMultipleChoiceAnswer(correctAnswerText) {
      ...
      for (const radio of radioInputs) {
        ...
        if (labelText && this.isAnswerMatch(labelText, correctAnswerText)) {
          ...
          await this.simulateHumanClick(radio);
          this.next();
          return true;
        }
      }
      await this.autoSelectRandomMultipleChoiceAnswer(); // Fallback to random selection if correct answer not found
      return false;
    }
    ...
    async autoSelectWrongMultipleChoiceAnswer(correctAnswerText) {
      const radioInputs = document.querySelectorAll('input[type="radio"]');
      const wrongOptions = [];
      ...
      if (wrongOptions.length > 0) {
        ...
        await this.simulateHumanClick(randomWrong.radio);
        this.next();
        return true;
      } else {
        // Fallback to correct answer if no wrong options found
        return await this.autoSelectMultipleChoiceAnswer(correctAnswerText);
      }
    }

    // Random selection methods
    async autoSelectRandomMultipleChoiceAnswer() {
      try {
        ...
        await this.simulateHumanClick(randomRadio);
        this.next();
    ...
    async autoClickNext() {
      try {
        ...
        await this.simulateHumanClick(nextBtn);
